/**
* ripped from here http://codepen.io/mikefab/full/IDdts/
...
BW.viz.IDdts = {
	...
	* 
	*/
	showViz: function(treeData) {
		//(function () {
		
		var construct_generations, diagonal, diameter, div, duration, height, i, radius, reconstruct_ancestors, reform_focus, root, store_and_update, svg, tree, update, zoom;
		//var treeData,
		
		//console.log('treeData'+treeData.valueOf());
		//console.log('treeData'+treeData.nodeType());
		console.log('treeData'+treeData[0].name);
		//console.log('RUN VIZ. treeData'+JSON.stringify(treeData, null, 2));
		
		
		window.current_nodes = [];
		update = function (source) {
			//console.log('RUN VIZ.update');	//JSON.stringify(params, null, 2)
			var link, links, node, nodeEnter, nodeExit, nodeUpdate, nodes;
			nodes = tree.nodes(root).reverse();
			links = tree.links(nodes);
			//console.log('RUN VIZ.update 1');
			nodes.forEach(function (d) {
				//console.log('RUN VIZ.update  d.depth'+d.depth);	
				return d.y = d.depth * 80;
			});
			//console.log('RUN VIZ.update 2'+JSON.stringify(nodes[0], null, 2));
			//console.log('RUN VIZ.update 2');
			node = svg.selectAll('g.node').data(nodes, function (d) {
				console.log('RUN VIZ.update d.id '+d.id);
				return d.id || (d.id = ++i);
			});
			//console.log('RUN VIZ.update 3');
			nodeEnter = node.enter().append('g').attr('class', 'node').attr('transform', function (d) {
				...
				//console.log('RUN VIZ.update 3 1');
			...
			//console.log('RUN VIZ.update 9');
			return nodes.forEach(function (d) {
				//console.log('RUN VIZ.update 9 1'+JSON.stringify(d.x, null, 2));
				d.x0 = d.x;
				return d.y0 = d.y;
			});
		};
		construct_generations = function (d) {
			//console.log('RUN VIZ.construct_generations');
			var c, generations;
			c = d;
			generations = [];
			while (c.parent) {
		...
		reform_focus = function () {
			//console.log('RUN VIZ.reform_focus');
			var count, d, set;
			if (window.current_nodes.length > 0) {
		...
		store_and_update = function (d) {
			//console.log('RUN VIZ.store_and_update');
		...
		reconstruct_ancestors = function (n, generations) {
			//console.log('RUN VIZ.reconstruct_ancestors');
			var count;
		...
		root.x0 = height / 2;
		root.y0 = 0;
		//console.log('RUN VIZ. update(root)'+JSON.stringify(root, null, 2));
		update(root);
	},
}
